$(document).ready(function() {
  ...
  function introRingLoad() {
    ...
    setTimeout(function() {
      ...
      sequenceFade(
        ".introRing:not(.introRing1)",
        "introRingGreenBorder",
        150,
        "remove"
      );
    }, 1200);
  ...
  $(".welcomeInfo").click(function() {
    // clicking infoIcon brings down information screen about the game
    $(".learnMore").removeClass("offScreenTop");
  });

  $("#backToTitle").click(function() {
    // send the info screen back up, off screen
  ...
  $(".choiceBox").click(function(e) {
    ...
    if (
      $(this)
        .siblings()
        .hasClass("mathSelected") === false
    ) {
      // on clicking choices, add selected class if no options are currently selected
      choice = this.id;
      setOperatorText(); // define selected game play operator
      $(this).toggleClass("mathSelected");
      if ($(".choiceBox").hasClass("mathSelected")) {
        // if an operator has been selected
        setTimeout(function() {
          // slide in the next container/pre-game option set of range
          $(".welcomeInfo").fadeOut();
  ...
  $(".rangeBox").click(function() {
    if (
      ...
      // on clicking a range, add selected class if no options are currently selected
      $(this).toggleClass("mathSelected");
      ...
      if ($(".rangeBox").hasClass("mathSelected")) {
        ...
        setTimeout(function() {
          // slide in the next container/pre-game option set of choosing difficulty
  ...
  $(".difficultyChoice").click(function() {
    if (
      ...
      // on clicking a difficulty, add selected class if no options are currently selected
      $(this).toggleClass("mathSelected");
  ...
  function loadWorldWelcome() {
    ...
    setTimeout(function() {
      // after sliding in the welcome screen
  ...
  function checkDynamicCSS() {
    if (!demo) {
      //DO NOT LOAD dynamic css if var demo === true.  Demo mode uses only world 1 and it is pre-loaded with styles.css
  ...
  function setWorldLevelNumber() {
    if (world < 5) {
      // on load of new level or game if world is 1-1 through 1-4
      if (level === 4 || world === 0) {
        // and if level is 4 on new load OR if it is a new game and world still = 0
        world++; // then advance a world (ex: after world 1-4, user goes to world 2-1)
      }
    }
    if (level < 4) {
      // if level is 1, 2, or 3
      level++; // then advance one level
    } else if (level === 4) {
      // if level is 4 though, then
      level = 1; // when that level has ended, reset var level to 1 because user is advancing to first stage of next world (world X-1)
    }
  }

  function setChances() {
    //use var difficulty to determine chances user has to answer incorrectly before game over
    if (level < 4) {
      switch (difficulty) {
        case "beginner":
          chances = 7;
  ...
  function setTimer() {
    //use var difficulty to determine time user has to earn the necessary rings to advance each level
    if (level < 4) {
      switch (
        difficulty // for levels 1, 2, 3 use difficulty to determine time
      ) {
        case "beginner":
          timer = 30;
          break;
      ...
      // level 4 is a bonus round to earn up to 10 rings in 10 seconds regardless of difficulty
      timer = 10;
    }
  }

  function setWorldRingRequirements() {
    // define how many questions must be answered correctly to earn a new ring
    switch (world) {
      case 1:
  ...
  function setLevelRingRequirements() {
    // define how many rings are required to advance to the next level
    switch (level) {
      case 1:
  ...
  function timedCount() {
    ...
    if (timer < 0) {
      // if timer hits 0
      if (level < 4) {
        // if level is < 4 (not bonus level)
        stopCount(); // stop the timer
        gameOver(); // run game over
      } else {
        // else if in the bonus round, running out of time does not end the game
        stopCount(); // stop the timer
  ...
  function randomizeFirstNum() {
    // use user range selection to randomize first number on game board within that arnge
    if (range === "to10") {
      range = 10;
    } else if (range === "to20") {
      range = 20;
  ...
  function randomizeSecondNum() {
    // set second number as random number from 1-10
    var secondNumber = parseInt(Math.floor(Math.random() * (10 - 1 + 1)) + 1);
    $("#secondNumber").text(secondNumber);
  }

  function getCorrectAnswer() {
    // determine the actual correct answer between the first and second number
    ...
    if (operator === "+") {
      // if operator is +, then add the two numbers for the correct answer
      answer = firstNum + secondNum;
    } else if (operator === "-") {
      // if operator is -, then subract the two numbers for the correct answer
      answer = firstNum - secondNum;
    } else if (operator === "*") {
      // if operator is *, then multiple the two numbers for the correct answer
      answer = firstNum * secondNum;
    }
    createChoicesArray(answer);
  }

  function createChoicesArray(answer) {
    // create choices for user: two randomly chosen incorrect answers and the correct answer provided via getCorrectAnswer()
  ...
  function shuffleChoicesArray(array, answer) {
    // shuffle the array of choices (2 incorrect, 1 correct) by randomizing the array element/index order
    for (var i = array.length - 1; i > 0; i--) {
      // Durstenfeld shuffle algorithm found via http://stackoverflow.com/questions/2450954/how-to-randomize-shuffle-a-javascript-array
  ...
  function setupChoices(array, answer) {
    // iterate over the .choices elements AND the shuffled array.
  ...
  $(".choices").click(function() {
    ...
    if (userChoice === answer) {
      ...
      if (highScore < totalPoints) {
        // if highScore is now < totalPoints
        highScore = totalPoints; // set highScore equal to totalPoints
        $("#highScore")
          .text("High: " + highScore)
          .addClass("pulse"); // set highScore element to new value of highScore and addClass pulse
      }
  ...
  function checkPoints() {
    // determines if rightAnswers is high enough to earn a new ring based on var forNewRing value (forNewRing value determined by World)
    ...
    if (rightAnswers === forNewRing) {
      // if rightAnswers is high enough forNewRing
      resetRightAnswers(); // reset rightAnswers to 0
    }
  }

  function resetRightAnswers() {
    // reset rightAnswers element content to 0
    rightAnswers = 0;
    setTimeout(function() {
  ...
  function checkRingsInRound() {
    // use var level value to determine if ringsInRound has increased enough to progress to next level
    switch (level) {
      case 1:
        if (ringsInRound === 4) {
  ...
  function prepNextLevel() {
    ...
    if (level < 4) {
      // if we aren't currently in level 4
      ...
      // if we ARE in level 4 then we need to load the next world
